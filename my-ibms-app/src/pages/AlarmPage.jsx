...
function AlarmRuleTab() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'level',
      width: 120,
      render: (l) => <Tag color={levelColor[l]}>{levelText[l]}</Tag>,
    },
  ...
  return (
    <div>
      ...
      <Modal
        title={editingRecord ? '编辑告警规则' : '创建新告警规则'}
        open={isModalOpen}
        onOk={handleModalOk}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item name="activateRule" label="激活规则" rules={[{ required: true, message: '请输入激活条件' }]}>
            <Input placeholder="例如: >= 30" />
          </Form.Item>
          <Form.Item name="resetRule" label="重置规则" rules={[{ required: true, message: '请输入重置条件' }]}>
...
function ActiveAlarmTab() {
  ...
  const columns = [
    ...
    { title: '寄存器', dataIndex: 'registerName', key: 'registerName', width: 150 },
    {
      ...
      key: 'currentValue',
      width: 100,
      render: (v) => <span style={{ fontWeight: 'bold', color: '#ff4d4f' }}>{v}</span>,
    },
    {
      title: '触发条件',
      dataIndex: 'threshold',
        ...
        <Button
          ...
          onClick={() => {
            setAlarms(alarms.map((a) =>
              a.key === record.key
                ? { ...a, status: '已结束', endTime: new Date().toLocaleString('zh-CN', {
                    year: 'numeric', month: '2-digit', day: '2-digit',
                    hour: '2-digit', minute: '2-digit',
                  }).replace(/\//g, '-') }
                : a
  ...
  return (
    <Card bordered={false}>
      <Table columns={columns} dataSource={alarms} pagination={false} />
    </Card>
  );
}

// ==================== 已结束告警事件 Tab ====================
function EndedAlarmTab() {
  const endedAlarms = [
    {
      key: '1',
      id: 'ALM-000',
  ...
  const columns = [
    ...
    { title: '结束时间', dataIndex: 'endTime', key: 'endTime', width: 160 },
    {
      ...
      key: 'status',
      width: 80,
      render: () => <Tag color="default">已结束</Tag>,
    },
  ];
...
export default function AlarmPage() {
  const tabItems = [
    { key: 'rule', label: '告警规则配置', children: <AlarmRuleTab /> },
    { key: 'active', label: '正在告警事件', children: <ActiveAlarmTab /> },
    { key: 'ended', label: '已结束告警事件', children: <EndedAlarmTab /> },
  ];
